...
rule run_rasqual:
    ...
    shell:
        ...
        while read peak; do
            ...
            # CRITICAL: Calculate actual window size for RASQUAL -w parameter
            # This is the actual genomic distance, not the config parameter
            ...
            echo "  Window: ${{window_start}}-${{window_end}}, WindowSize: ${{actual_window_size}} bp" >> {log}
            echo "  cisSNPs: ${{num_cisSNPs}}, peakSNPs: ${{num_peakSNPs}}" >> {log}
            
            # Run RASQUAL with actual calculated window size
