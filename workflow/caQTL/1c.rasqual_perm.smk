...
rule run_rasqual_permutation:
    ...
    shell:
        ...
        while read peak; do
            ...
            echo "  Window: ${{window_start}}-${{window_end}}, cisSNPs: ${{num_cisSNPs}}, peakSNPs: ${{num_peakSNPs}}" >> {log}
            
            # Run RASQUAL with permutation (-r flag)
...
rule combine_permutation_results:
    input:
        chr_results=expand(OUT("caQTL", "rasqual_permutation", "window_{{window}}kb", "pc{{pc}}",
                              "{{condition}}_{chr}_pc{{pc}}_{{window}}kb_perm{{perm}}.txt"),
                          chr=CHROMOSOMES)
    output:
