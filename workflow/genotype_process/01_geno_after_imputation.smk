...
rule qcVCF:
    ...
    shell:
        ...
        plink --vcf {input} --threads {threads} --const-fid 0 --geno {params.geno} --hwe {params.hwe} --maf {params.maf} --recode vcf --out output/vcf/{params.prefix}_ALL_qc_temp 1> {log.out}
        mv output/vcf/{params.prefix}_ALL_qc_temp.log output/vcf/logs/{params.prefix}_ALL_qc.err

        awk '!/^#/{{print $3}}' output/vcf/{params.prefix}_ALL_qc_temp.vcf > filtered_variants.txt
        bcftools view -i 'ID=@filtered_variants.txt' {input} -o output/vcf/{params.prefix}_ALL_qc.vcf -O v
        rm output/vcf/{params.prefix}_ALL_qc_temp.nosex
