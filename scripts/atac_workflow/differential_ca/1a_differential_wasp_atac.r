...
# date edited: 09/23/2025
# WASP deseq2 ATAC-seq using macs2
#--------------------------------------------------------
...
bamqc_dir <- file.path(base_path, "07_qc/ataqv")
...
#----- Loading data ------
macs2_df <- read_and_clean_peaks("wasp/peaks/counts/all_peaks_combined_counts.txt")

# Take the replicate and femur out (Double check)
#macs2_clean_df  <- macs2_df[, !grepl("Femur|replicate", colnames(macs2_df))]
...
save(se, macs2_gr, counts_macs2, meta_final, file = file.path(out_data_dir, "00_macs2_wasp_prep_dseq2data.RData"))
...
save(cqn.counts, cqnNormFactors,GCcontent,peakwidths,
     file = file.path(out_data_dir, "01_macs2_wasp_cqn_normFactors.RData"))
